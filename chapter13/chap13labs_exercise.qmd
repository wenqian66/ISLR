...
#First we create 100 variables, each consisting of 10 observations. The frst 50 variables have mean 0.5 and variance 1, 
...
# compute the 100 pvalues, and then construct a vector recording whether the jth p-value is less than or equal to 0.05
...
#Applied 7.
```{r}
qvariables <- c("CompPrice", "Income", "Advertising", "Population", "Price", "Age", "Education")

results <- list()
p_values <- vector()

for (var in qvariables) {
  formula <- as.formula(paste("Sales ~", var))
  model <- lm(formula, data = Carseats)
  results[[var]] <- summary(model)
  
  p_value <- coef(summary(model))[paste(var), "Pr(>|t|)"]
  p_values <- c(p_values, p_value)
  
  cat(sprintf("P-value for the coefficient of %s: %f\n", var, p_value))
}

carseats_pvals <- as.numeric(p_values)
```
```{r}
carseats_pvals <= .05
```
...
```{r}
alpha <-  0.05
n_tests <-  length(carseats_pvals)

bonf_pvals <- p.adjust(carseats_pvals, method = "bonferroni")
reject <- bonf_pvals < (alpha / n_tests)
reject
```
...
```{r}
q.values.BH <- p.adjust(carseats_pvals, method = "BH")
q.values.BH < alpha
```
...
hist(p.values, main = "Histogram of P-Values", xlab = "P-Value", breaks = 10, col = "blue")
```

```{r}
alpha <- 0.05
decision <- ifelse(p.values < alpha, "Reject H0", "Do not reject H0")
sum(decision == "Reject H0")
```

```{r}
bonferroni <- p.adjust(p.values, method = "bonferroni")
decision_fwer <- ifelse(bonferroni < alpha, "Reject H0", "Do not reject H0")
sum(decision_fwer == "Reject H0")
```


```{r}
fdr <- p.adjust(p.values, method = "BH")
decision_fdr <- ifelse(fdr < alpha, "Reject H0", "Do not reject H0")
sum(decision_fdr == "Reject H0")
```

```{r}
indices <- order(p.values)[1:10]

# FWER control
p.values_top10 <- p.values[indices]
bonferroni_top10 <- p.adjust(p.values_top10, method = "bonferroni")
decision_fwer_top10 <- ifelse(bonferroni_top10 < alpha, "Reject H0", "Do not reject H0")
sum_reject_fwer_top10 <- sum(decision_fwer_top10 == "Reject H0")

# FDR control
fdr_top10 <- p.adjust(p.values_top10, method = "BH")
decision_fdr_top10 <- ifelse(fdr_top10 < alpha, "Reject H0", "Do not reject H0")
sum_reject_fdr_top10 <- sum(decision_fdr_top10 == "Reject H0")

print(sum_reject_fwer_top10)  
print(sum_reject_fdr_top10) 
```
